...
void loop() {
  ...
  if (!digitalRead(sensorPin)) {
    ...
    while (!digitalRead(sensorPin)) {
      // When the IR sensor is triggered(means if the sensor detects the drowsiness), turn on the buzzer and turn of the motor and wait for 1 second
    ...
    if (TimeDelay() >= 4) {
      digitalWrite(motorPin, HIGH);  // If 3 seconds have passed, turn on the motor
    }
    if (TimeDelay() >= 2) {
      digitalWrite(buzzerPin, LOW);   // If 4 seconds have passed, turn off the buzzer
    }
  }
}
